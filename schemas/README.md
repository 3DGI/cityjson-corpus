...
| `outputs[].producer` | free-form strings such as `upstream`, `cjio`, `cityjson-corpus` | Names the system that produced the artifact. This distinguishes upstream source files from locally derived benchmark formats.                                                                          |
| `outputs[].derivation` | `acquired`, `merged`, `exported`, `materialized`, `subset` | `acquired` means the artifact is preserved from the source dataset, `merged` means multiple source artifacts were combined, `exported` means a sibling format was written from another artifact, `materialized` means a corpus pipeline produced a benchmark layout, and `subset` means a smaller fixture was selected from another artifact. |
